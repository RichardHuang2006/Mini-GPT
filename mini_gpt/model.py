"""Decoder-only Transformer: GQA, RoPE, QK-norm, RMSNorm, SwiGLU, sliding windows."""
...
# Targets set to IGNORE_INDEX drop out of the loss; this is how SFT restricts
# the loss to assistant tokens.
IGNORE_INDEX = -100
...
def precompute_rope(
    ...
    *,
    ...
    """Return RoPE (cos, sin), each [T, head_dim]. Pair i spins at base^(-2i/d)."""
...
def rotate_half(x: torch.Tensor) -> torch.Tensor:
    """Swap the halves of the last dim and negate the first: apply_rope's partner."""
    x1, x2 = x.chunk(2, dim=-1)
    return torch.cat([-x2, x1], dim=-1)


def apply_rope(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    """Rotate x [B, H, T, d] by position; cos/sin [T, d] broadcast over B and H."""
    cos = cos[None, None, :, :]
    sin = sin[None, None, :, :]
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    """Expand [B, Hkv, T, d] -> [B, Hkv * n_rep, T, d] for grouped-query attention."""
    if n_rep == 1:
        return x
    b, n_kv, t, d = x.shape
    return (
...
def build_attn_mask(
    ...
    """Boolean [T, T] mask, True where query i may attend to key j."""
...
class Attention(nn.Module):
    """One layer: project -> QK-norm -> RoPE -> GQA expand -> SDPA."""
    ...
    def forward(
        ...
        if seg_equal is not None:
            # The causal diagonal always survives, so no row is fully masked --
...
class SwiGLU(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        a = self.gate(x)
        b = self.up(x)
        h = kernels.swiglu(a, b) if self.use_triton else F.silu(a) * b
        return self.down(h)
...
class Block(nn.Module):
    ...
    def __init__(self, cfg: Config, layer_idx: int):
        super().__init__()
        self.attn_norm = RMSNorm(cfg.d_model, use_triton=cfg.use_triton)
        self.attn = Attention(cfg, layer_idx)
        self.mlp_norm = RMSNorm(cfg.d_model, use_triton=cfg.use_triton)
        self.mlp = SwiGLU(cfg.d_model, cfg.mlp_hidden, use_triton=cfg.use_triton)
...
class MiniGPT(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        self.final_norm = RMSNorm(cfg.d_model, use_triton=cfg.use_triton)
        self.lm_head = nn.Linear(cfg.d_model, cfg.vocab_size, bias=False)  # D -> V

        # Opt-in chunked cross-entropy: skips the [B*T, V] logits and returns
    ...
    def _scale_residual_projections(self) -> None:
        # GPT-2 residual growth control: bound the residual variance with depth.
    ...
    def num_parameters(self) -> int:
        """Unique parameter count; the tied embedding counts once."""
        seen: set[int] = set()
        total = 0
